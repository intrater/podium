...
| U4 | Per-episode Claude pipeline (with content sign-off gate) | **in progress — Stage 2 module built; Stage 3 (pipeline integration) is next** | Biggest lever. Stage 0 (2026-05-13): baseline ~$0.90/team/day, above $0.40 downscope threshold → proceed. Stage 1 (2026-05-13): full-episode transcript fetch + per-episode Claude call is the path; inline `windows[].lines[]` alone is too narrow for substantive summaries. Stage 1.5 (2026-05-13): v0 A/B run across 4 existing cards. User signed off after reviewing card [1] — quality equivalent on small/medium episodes. Stage 2 (2026-05-13): **built** the production module `lib/anthropic/extract-episode-moments.ts` (10K LOC ish) + new types (EpisodeExtractionInput, EpisodeMoment, EpisodeExtractionOutput, TranscriptLine, MentionAnchor) + new system prompt at `lib/anthropic/prompts/episode-extraction.ts` (verified 4,384 system tokens — clears Haiku 4.5's 4,096 cache minimum). Verified live: cache fires (call 1 cache_creation=4646, call 2 cache_read=4646). 7 new tests pass (happy path, empty-moments, quote fidelity reject+retry+null, unknown-anchor reject+retry, schema recovery, cache_control + max_tokens=4096 invariants). 186 tests total passing. Build clean. **U2 will activate automatically once Stage 3 wires this into the pipeline.** Stage 3 next: refactor `pipeline.ts:137–171` from per-segment fan-out to per-episode fan-out, persistence-shape mapping preserved (each moment → segment row via particle_segment_id), then user sign-off gate via `inspect-card`. |
