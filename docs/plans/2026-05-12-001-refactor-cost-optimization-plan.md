...
| U4 | Per-episode Claude pipeline (with content sign-off gate) | **in progress — Stage 0 + 1 done, Stage 1.5 awaits user verdict** | Biggest lever. Refactor pipeline.ts:137–171 from per-segment fan-out to per-episode fan-out. Stage 0 (2026-05-13): baseline confirmed at ~$0.90/team/day; well above $0.40 downscope threshold → proceed. Stage 1 (2026-05-13): full-episode transcript fetch + per-episode Claude call is the path; inline `windows[].lines[]` alone is too narrow for substantive summaries; the same Particle endpoint serves both clip and episode transcripts at $0.008/call so episode-level is a 6× call-count reduction. Stage 1.5 (2026-05-13): v0 A/B run across 4 existing cards (Particle credits depleted, so used cached `segments.raw_transcript`). Quality comparable on cards [1] (5 segs → 5 moments) and [2] (7 → 6, NEW dropped a borderline match). Card [3] hit max_tokens=2048 cap on 11-segment episode — fix is `max_tokens: 4096` in Stage 2. Cache still dormant (v0 prompt ~3,000 tokens, below 4,096). Cost: $0.043. Full output: `docs/solutions/2026-05-13-ab-output.txt`. Verdict captured in `docs/solutions/2026-05-12-episode-extraction-prompt.md`. **Persistence design (committed):** extraction prompt preserves Particle segment boundaries — each moment carries its `particle_segment_id`, existing UNIQUE constraint + `onConflict` upsert path unchanged. CRITICAL: user-approval gate on new card shape via `inspect-card` + `qa:screenshots`. Multiple prompt iteration cycles expected within the unit. |
