---
date: 2026-05-12
status: substantially complete — U4 final user sign-off pending tomorrow's natural cron cycle; U7 deferred as optional
plan-id: 2026-05-12-001
type: refactor
title: "Drive Podium ingest cost to ≤$0.20/team/day"
origin: docs/strategy/unit-economics.md
revised: 2026-05-13 (full execution + reliability shakeout in one session — U1/U2/U3/U4/U5/U6/U8 all shipped; U7 deferred)
prior-revisions:
  - 2026-05-13 (late) — Live validation run surfaced three reliability issues that the unit tests didn't catch. All three fixed in commit `a529792`: (1) Quote fidelity moved to graceful degradation — a bad pull quote drops just that quote, not the entire episode. (2) Podcast-catalog filter added in the pipeline — entity-mention searches return content from all of Particle's universe, not just our 31 curated podcasts; off-catalog segments now drop before paying for transcripts/Claude. (3) Concurrency reduced from 5 to 1 + Anthropic SDK maxRetries=5 to fit inside Tier-1's 50K-tokens/min rate limit. Also added `?limit=N` query param on POST /api/ingest for cheap validation runs. Validated: ingest with limit=5 produced 1 real card (5-segment Locked On 49ers episode) with no silent failures; subsequent run dropped 28 off-catalog segments cleanly at $0.30 cost vs the prior $0.97. AnthropicQuoteFidelityError class removed as dead code.
  - 2026-05-13 (mid) — U4 Stage 0/1/1.5/2/3 all executed; U5 (prompt versioning) + U6 (cadence policy) both shipped end-to-end. Two migrations applied to live DB: `0013_teams_cadence.sql` (U6, cadence_days column + iteration of teams from DB in cron) and `0014_segments_prompt_version.sql` (U5, prompt_version column with `'legacy'` backfill). `extract-episode-moments.ts` shipped + tests + verified caching fires at 4,384 prefix tokens (call 1 cache_creation=4646 → call 2 cache_read=4646). Pipeline refactored to per-episode fan-out; legacy `summarize.ts`/`summarize-episode.ts`/`prompts/segment-summary.ts` deleted; shared helpers moved to `lib/anthropic/_helpers.ts`. Stage 1.5 A/B validated quality equivalence on cards [1] and [2] vs the legacy per-segment pipeline; user signed off on content shape.
  - 2026-05-13 (early) — Diagnosed the U2 cache miss. Root cause: Claude Haiku 4.5's documented minimum cacheable prefix is **4,096 tokens**, not the 2,048 the plan assumed. The marker placement was correct; the four "ranked suspects" in the prior revision were all wrong trees. Solutions doc at `docs/solutions/2026-05-13-anthropic-haiku-4-5-cache-minimum.md`. Decision: don't pad `SHARED_RULES` purely to clear the threshold; U4's per-episode extraction prompt is sized at 4,384 tokens and clears the minimum naturally. U2 activates automatically the first time U4 runs live.
  - 2026-05-13 (overnight) — Phase A `/ce-work` autonomous execution. Four commits on main pushed to origin. U2 (caching fix) shipped structural marker placement on system + tools per Anthropic SDK types, 5 new unit-test assertions pass; live verification run reported 0% cache hit rate (root-caused that day — see entry above). U1, U3, U8 verified end-to-end on live data.
...
| U4 | Per-episode Claude pipeline (with content sign-off gate) | **substantially complete — final card-shape sign-off pending tomorrow's natural cron cycle** | Biggest lever. Stage 0 (2026-05-13): baseline ~$0.90/team/day, above $0.40 downscope threshold → proceed. Stage 1: full-episode transcript fetch + per-episode Claude call is the path. Stage 1.5: v0 A/B validated quality-equivalent on cards [1] and [2]. User signed off. Stage 2: **built** `lib/anthropic/extract-episode-moments.ts` + types + system prompt (4,384 tokens, clears 4,096 cache minimum, verified cache_creation=4646 → cache_read=4646). Stage 3: **refactored `pipeline.ts`** to per-episode fan-out; one `getClipTranscript` call per episode + one `extractEpisodeMoments` call per episode → moments mapped to segment rows via `particle_segment_id`. Cleanup: deleted now-unused `summarize.ts`/`summarize-episode.ts`/`prompts/segment-summary.ts`/`summarize.test.ts`; shared helpers moved to `lib/anthropic/_helpers.ts`. **Reliability shakeout (commit `a529792`):** validation runs surfaced three real issues that unit tests didn't catch. (1) Quote fidelity → graceful degradation: bad pull quotes drop just that quote, never the whole episode. (2) Podcast-catalog filter added so off-list entity matches drop before Anthropic spend. (3) `EPISODE_CONCURRENCY=1` + Anthropic SDK `maxRetries=5` to fit Tier-1's 50K-tokens/min rate ceiling. Plus `?limit=N` query param for cheap validation runs. Latest live run: 1 real card from a 5-segment Locked On 49ers episode (Aiyuk/Warner/Greenlaw drama) — clean, no silent failures. **Final gate (pending):** tomorrow's natural cron (or another manual run when more catalog content has been published) surfaces fresh cards; user reviews and either approves or directs prompt iteration. 175 tests pass; build + lint clean. |
...
**The cost-optimization plan is substantially complete.** All eight units' code is in. Particle credits have been restored (Anthropic credits too). One real validation run has produced a clean card under the new pipeline. The remaining work is observational + one user judgment call.

What's left:

1. **Watch one natural cron cycle** (tomorrow's 11:00 UTC scheduled_run). If any of the 31 catalog podcasts has published a new episode in the lookback window, the cron will produce one or more fresh cards under the new per-episode pipeline + U5 prompt-versioning + U6 cadence gate. Open `npm run inspect-card -- all` to review.

2. **User reviews card shape and signs off (or directs prompt iteration).** This is the final U4 gate. If the shape is right, U4 is fully complete. If not, bump the prompt in `lib/anthropic/prompts/episode-extraction.ts` AND increment `EPISODE_EXTRACTION_PROMPT_VERSION` in `lib/anthropic/types.ts` — U5 will auto-reprocess the existing segments on the next run.

3. **U2 final verification happens automatically** the moment a live ingest runs through the new pipeline. Inspect via `npm run inspect-costs -- since=<run date>` — Anthropic detail should show `cache_read_input_tokens > 0` on call 2 onward (call 1 primes the cache). System prompt is already verified at 4,384 tokens via `scripts/debug-cache.ts`.

4. **U7 is optional, currently deferred.** Annual-average cost projection from U4 + U6 is ~$0.18/day, comfortably below the $0.20 target. Only worth running U7 (model swap evaluation) if real cost numbers over a 30-day window come in above target. Otherwise, save the eval spend.

What we know works end-to-end (validated in the 2026-05-13 session):
- Per-episode extraction produces high-quality cards (the 5-segment Locked On 49ers card was substantively comparable to per-segment baseline).
- Anthropic prompt caching fires (cache_creation 4,646 tokens on call 1, cache_read 4,646 on call 2 in isolation tests).
- Cadence policy short-circuits scheduled runs during off-season (verified in unit tests; will fire in production on the next off-season day).
- Prompt version filter re-processes legacy segments when the prompt version bumps (verified in tests; legacy backfill ran during migration 0014).
- Catalog filter drops off-list entity matches before Anthropic spend (latest run dropped 28 segments cleanly at $0.30 vs prior $0.97 wasted runs).
- Quote-fidelity graceful degradation keeps cards alive when the model lightly rephrases a quote.

### What's blocked on the user

1. **U4 final card-shape sign-off** — open `npm run inspect-card -- all` after tomorrow's cron (or trigger a manual run when more catalog content has been published) and approve the shape or direct prompt edits.
2. **Particle catalog gap (optional)** — today's validation run dropped 28 segments from podcasts not in our 31-podcast curated list. Some may be worth adding (e.g., Pardon My Take, Bill Simmons Pod) if they regularly produce 49ers content. Pure content decision; doesn't block anything.
3. **Model-swap decision in U7 (only if needed)** — only triggers if 30-day real-cost window comes in above $0.20/team/day. Annual projection says we're already below.

Nothing else blocked on user.

### Deferred items (outside the cost-opt plan, captured for future)

- `podiumsports.app` DNS isn't pointing at Vercel yet — separate task from the v1 plan. Not blocking until you're ready to share the URL.
- Multi-tenant ingest sharing (v2) — required for the $3.99/month economics to actually work at multi-user scale. Separate plan: `docs/strategy/unit-economics.md` §"How multi-tenant sharing makes the math work" has the breadcrumb.
